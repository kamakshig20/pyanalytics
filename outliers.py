...
data('Boston') #https://www.kaggle.com/c/boston-housing to understand dataset
boston = data('Boston')
...
dfTF.sum(axis=0) #column wise number of outliers
dfTF.sum(axis=1) #rowwise number of outliers
...
dfTF[(z<2).all(axis=1)].shape #all rows with z values<2, not outliers

boston_wo = boston[(z < 3).all(axis=1)] #rows with z<3, keeping only non outliers
boston_wo.head()

boston.shape #shape of original data
boston_wo.shape #shape of data without outlier

boston_wo2 = boston[~((boston < (Q1 - 1.5 * IQR)) |(boston > (Q3 + 1.5 * IQR))).any(axis=1)]
boston_wo2.shape
sns.boxplot(x=boston_wo2.crim)
